// @ts-check
import { isMain } from "../helpers.mjs";
    ...
    for (const [key, value] of entries) {
        switch (typeof value) {
            ...
            case "object":
                if (Array.isArray(value)) {
                    lines.push(
                        `${innerIndent}${str(key)}: ${array(value, level + 1)},`,
                    );
                } else if (value) {
                    lines.push(
                        `${innerIndent}${str(key)}: ${object(value, level + 1)},`,
                    );
                } else {
...
function components(components, level) {
    ...
    for (const c of components) {
        ...
        lines.push(
            `${innerIndent}displayName: ${str(c.displayName ?? c.appKey)},`,
        );
        lines.push(
            `${innerIndent}initialProperties: ${object(c.initialProperties, level + 2)},`,
        );
        lines.push(
            `${innerIndent}presentationStyle: ${str(c.presentationStyle)},`,
        );
